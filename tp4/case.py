from tp4.caseMultiplicateurInvalideException import CaseMultiplicateurInvalideException
from tp4.caseEffetIncorrectException import CaseEffetIncorrectException
...
class Case:
    ...
    def __init__(self, multiplicateur=1, effet=None):
        ...
        # Exception pour multiplicateur invalide
        if not(1 <= multiplicateur <= 3):
            raise CaseMultiplicateurInvalideException('Multiplicateur incorrect.')

        # Exception pour effet invalide
        if not(effet is None or effet in 'ML'):
            raise CaseEffetIncorrectException('Type incorrect.')
