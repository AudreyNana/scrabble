...
from tp4.preconditionInsatisfaiteException import PreConditionInsatisfaiteException
from tp4.utils import coordonnees_case, dessiner_jeton
from tp4.positionInvalideException import PositionInvalideException
from tp4.choixUnitaireLigneOuColonneException import ChoixUnitaireLigneOuColonneException
...
class Plateau(Canvas):
    ...
    def decode_position(self, code):
        ...
        # Exception pour la position invalide
    ...
    def placer_mots(self, jetons_a_ajouter, position_codes):
        ...
        # Exception pour précondition insatisfaite
        if (len(jetons_a_ajouter) != len(position_codes)):
            raise PreConditionInsatisfaiteException('Le nombre de jetons est différent du nombre de positions.')

        # Exception pour position invalide
    ...
    def mots_et_score_sur_ligne_ou_colonne(self, nouvelles_positions, ligne=None, colonne=None):
        ...
        # Exception pour choixUnitaire ligne ou colonne
        if not ((ligne is None) ^ (colonne is None)):
            raise ChoixUnitaireLigneOuColonneException('Précisez seulement la ligne ou la colonne, pas les deux.')
