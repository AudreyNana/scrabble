from tp4.lettreIncorrectException import LettreIncorrectException
from tp4.valeurJetonIncorrectException import ValeurJetonIncorrectException
...
class Jeton:
    ...
    def __init__(self, lettre, valeur):
        ...
        #Exception pour lettre incorrecte
        if not(len(lettre) == 1 and lettre.isupper() and lettre.isalpha()):
            raise LettreIncorrectException('Lettre incorrecte.')

        #Exception pour la valeur incorrecte du jeton
        if not(0 <= valeur <= 20):
            raise ValeurJetonIncorrectException('Valeur incorrecte.')
